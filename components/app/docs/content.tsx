...
const ROUND_SEMANTICS: { mode: string; round: string; ends: string; runEnds: string }[] = [
  { mode: "Agora", round: "One lead opens (rotating each round) + replies routed to ANY post from the round, weighted toward recent and contested — focused: up to 6 replies · lively: ~1.5× leads · bustling: 2× leads", ends: "Reply budget for the round is spent → crowd poll → interjections → votes", runEnds: "Rounds cap · positions stable twice in a row (from round 3) · post budget" },
  ...
  { mode: "Jury", round: "Every juror scores (blind in round 1; sees the tally after) + a code-computed TALLY — density never adds deliberation posts here; verdict integrity is the mode", ends: "Tally posts → crowd poll → interjections → votes", runEnds: "Scores stop moving (no juror shifts ≥1 point) · rounds cap · post budget" },
  ...
  return (
    <>
      ...
      <Steps items={[
        ...
        { title: "Corpus", body: "Upload your documents — surveys, offering memos, zoning letters, rent rolls, photos. Agents cite them by name and page; images are named too, so 'describe 4.jpg' just works, image rows show thumbnails with a click-to-view preview, and typing @ in Test-the-corpus inserts an exact filename. Files the decision turns on (the winning listing photo, the key plan page) can appear inside the report itself. No documents? Write what you know and it becomes one." },
...
function InteractionModes() {
  return (
    <>
      ...
      <RoundSemanticsTable />
      <P><B>What the poll actually asks.</B> At launch, the engine turns your brief into one plain-language poll question — a proposition a bystander can support or oppose ("Should the town let the data center be built?") — and every round's poll asks that same question, so the percentages always have a clear referent. The question is displayed on each poll card and in the report. Each polled crowd member answers <B>in character</B>, and — critically — after hearing a digest of what the panel argued that round, so movement between rounds is reaction to the debate, not noise. The four stances are defined, not vibes: <B>support</B> = would say yes · <B>conditional</B> = yes, if a named concern is handled · <B>oppose</B> = would say no · <B>disengaged</B> = genuinely unaffected (someone with no strong opinion counts as conditional, never disengaged).</P>
      <P>Every polled round closes the same way: <B>crowd poll</B> (the sentiment bar) → <B>interjection burst</B> (a few crowd members post short reactions threaded under the round's posts) → <B>vote pass</B> (every lead plus a crowd sample endorses ▲ or rejects ▼ the round's arguments, in character — hover a vote chip to see exactly who). At lively and bustling, votes also accumulate <B>in realtime</B>: a rotating sample reacts every few posts, so chips move while the round is still arguing, and the close completes the sweep — each voter votes each post at most once. A poll interrupted by a slice boundary ships its partial tally honestly, and nothing is ever double-polled, double-burst, or double-voted on resume.</P>
