...
function InteractionModes() {
  return (
    <>
      ...
      <P>When your brief has been read by the understanding pass, rounds stop being undirected passes. Each round opens with an <B>agenda</B>: round 1 opens positions across the full brief, middle rounds target the <B>least-resolved sub-asks by name</B> ("Round 3 focus: 'who are the players?' — still missing: nobody named one"), and the final round forces the panel to commit to the answer. Behind the rounds, a <B>resolution tracker</B> scores every sub-ask 0–100 after each close, and the run screen carries the <B>DOCKET</B> — a slim rail docked on the agent-network canvas, one mini score bar per question plus the aggregate. Click it and it expands into the full docket: every question in <B>full text</B> (nothing truncates) with its 0–100 score, a fill bar, the tracker's "still missing" note inline, and the current round's agenda as a card on top — so you can see the deliberation systematically covering what you asked, and convergence means something. A bar ticks once when its score moves; the rail remembers whether you keep it open, per simulation. Round dividers in the feed carry each round's agenda. The mode's choreography is untouched — the agenda rides inside the opening instruction.</P>
