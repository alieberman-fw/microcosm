...
function LeadsAndTheCrowd() {
  return (
    <>
      ...
      <Term term="Polled every round">Every crowd member answers the poll, every round — the question is derived from your brief (a choose-between brief polls your actual options), and with an adaptive plan the question CHANGES with the run's arc (a gut-read early, the decision-shaped closer late). Percentages always sum to 100, raw counts sit beside them, EVERY VOTE expands the individual ballots — and the crowd's read RETURNS to the panel: next round's speakers open knowing what the population thinks. Ballots are validated against the roster (an invented name or a double answer never counts), and a poll cut short by a time slice is labeled PARTIAL and re-run in full — never passed off as the round's sentiment. If a panelist's turn ever produces nothing usable, the feed shows a quiet "skipped a turn" marker instead of silently reflowing around the hole.</Term>
