...
- **Shipped (POC status) — grounding architecture decided:** v1 passes **whole documents directly to Claude** instead of embeddings-RAG. Upload (`/api/documents`) stores the original in Storage, extracts text locally (unpdf) into `doc_chunks` (Postgres FTS substrate — no embeddings yet), uploads to the **Anthropic Files API**, and records an exact `count_tokens` figure per doc. Q&A and (soon) agents reference docs by `file_id` as native document/image blocks with **citations enabled** (page-level for PDFs) and a `cache_control` breakpoint on the corpus prefix, so repeat calls read the corpus at ~0.1× price. Model steps up to the 1M-context tier when the corpus exceeds `DIRECT_CONTEXT_BUDGET` (150K tokens) or any PDF exceeds 100 pages (`lib/corpus.ts` is the config home — never hardcode). Chunk **retrieval** (FTS now; pgvector hybrid when activated) kicks in for swarm-scale runs and the per-claim verifier, where full-corpus context per agent would blow the run budget. **Embeddings provider decided: the Vercel AI Gateway** (OpenAI-compatible `POST https://ai-gateway.vercel.sh/v1/embeddings`, `AI_GATEWAY_API_KEY` env var, billed through the existing Vercel account; default model `openai/text-embedding-3-small`, whose 1536 dims match the existing `vector(1536)` columns — config in `lib/corpus.ts`). The brief composer (`/sim/new`) + simulation workspace (`/sim/[id]`) ship with this; "Test the corpus" on the workspace proves the cited-answer path end to end (answers render as one markdown block — GFM tables supported — with a deduped CITED chip row). **Field notes**: users with no documents write what they know in the workspace ("✎ WRITE WHAT YOU KNOW") and it becomes a real corpus document through the same pipeline — parsed, chunked, Files-API-uploaded, citable by name.
...
**Shipped (POC status):** the §3 composition surface lives on the population stage: composition selector (AUTO lets the director decide; EXPERTS / RESIDENTS / MIXED force it — pre-cast pills or a two-step re-cast after), a seven-mode selector persisted via `PATCH /api/simulations/[id]/config` with YOURS vs RECOMMENDED labeling, editable full-run crowd counts (experts 4–500 · residents 0–1,000, `config.casting.scale` + `user_set` flags — the engine's §4.1 inputs), and a **CrowdBand** dot-field under the lead cards so the whole population (leads + crowd) is visible in the population space. The Casting Director is live on `/sim/[id]` (`POST /api/simulations/[id]/cast`, ND-JSON stream). One `CASTING_MODEL` pass (Sonnet minimum — `lib/casting.ts`) over the brief + corpus excerpts emits the plan — composition, recommended full-run scale, recommended interaction mode with rationale, and 6–12 lead seats with exactly one adversarial seed — then each seat resolves live on the Seed-screen card grid (shimmer → card): matched against the **org's custom personas first** (token-overlap), then the **global library** (`search_personas` FTS on the seat's query, then its role), and only true gaps are **generated** (batched, name-collision-avoiding) and saved to the org's custom library (`source 'auto'`) so the catalog self-heals. Cards carry provenance tags (YOUR LIBRARY / LIBRARY MATCH / GENERATED / ADVERSARIAL SEED), open the full persona profile, and are removable; a guidance line re-casts the panel ("more first-time buyers; add a school-board voice"). Every seat freezes into `sim_agents.spec_frozen` (with seat role/why/discipline/provenance; adversarial seats get an explicit opposition mandate appended to their stances) and the plan lands in `simulations.config.casting`. Both model calls are logged to `agent_interactions` (`casting.plan`, `casting.generate`); both parses run through `lib/llm-json.ts` (bracket-stack truncation salvage + one silent retry; plan cap 3,500 tokens). The pre-cast choice is **auto-cast vs hand-pick** (SeatPicker modal: org personas + library smart search, zero model calls — also "+ ADD FROM LIBRARY" after any cast, via `POST /api/simulations/[id]/agents`). While the plan drafts, the **casting theater** plays the §5 node/pulse grammar (canvas dots forming the panel ring, warn-colored adversarial dot, rotating status lines); seat events carry the FROZEN spec so live cards show seat roles, disciplines, and adversarial tags mid-cast. pgvector matching upgrades the FTS step when embeddings activate (§2 Stage 2). **Crowd materialization (shipped):** the §4.1 full-run crowd is real members on the population stage, not a number — `POST /api/simulations/[id]/crowd` generates compact Haiku-tier personas (`CROWD_MODEL`/`CROWD_BATCH`/`CROWD_SAMPLE_CAP` in `lib/casting.ts`; 3-concurrent batches, name-deduped, wrapper-salvage + zero-produce retry, logged as `crowd.generate`) into `sim_agents` with `seat.tier "crowd"` and `persona_id` null; the CrowdBand lights dots as members land; the CrowdRoster modal browses/filters/removes every member; crowds beyond the cap get a proportional representative sample (full scale instantiated at run); residents are narrative-seeded until ACS PUMS lands, and the engine treats crowd rows as the §5 sampled-interjector/sentiment-poll population.
...
**Shipped (POC status):** pre-generation is implemented as `scripts/generate-personas.mjs` — parses `docs/persona-taxonomy.md`, generates full persona JSONs with the Sonnet tier (10 seats per call; idempotent via `spec.seed_key`, safe to resume/re-run), and writes global library rows (`org_id` null, `public`, `source 'library'`); §28–30 household/demand cohorts get 2–3 demographic variants each (~1,870 personas). Library search is **LLM smart search** (`/api/personas/search`): a Haiku pass translates natural language ("under 40 homeowner", "looking to build a data center") into a websearch-syntax full-text query plus structured demographic filters, executed by the `search_personas` RPC (Postgres FTS over a generated `search` tsvector + jsonb filters, RLS-scoped, plain-FTS fallback if the parse fails; the Haiku call is logged to `agent_interactions` as `library.search`). The §3.3 pgvector matching flow activates when an `AI_GATEWAY_API_KEY` is added (embeddings via the Vercel AI Gateway, §2 Stage 2) — `personas.embedding vector(1536)` is already in the schema.

**Curation (QC, shipped):** `scripts/curate-personas.mjs` — `report` (name stats + programmatic red flags), `fix-names` (full names are unique library-wide; renames rewrite first-name references in backstory/tagline/stances), `audit` (Haiku pass over every persona checking income vs role+metro, years_experience vs age and tagline, credentials vs role, tenure vs cohort, metro vs state; fixes the *fields* to match the narrative, never the narrative; report → `scripts/audit-report.json`). The generator enforces name uniqueness on all future runs. In Conversations, two participants sharing a display name stay distinct end-to-end: the @mention typeahead sends resolved persona keys with the message, and transcripts label duplicate names with roles ("Bob G. (DSCR lender)").
...
- **Conversations (direct chat — shipped early).** The second product surface, complementary to simulations: where a simulation is agents deliberating *autonomously* in the forum UI (with the user taking the floor at the edges), Conversations is the user talking *directly* with personas in an iMessage-style thread. Shipped behavior:
  - **1:1 and group threads.** Any mix of library and custom personas, up to 20 per conversation. Threads can be renamed or deleted from a hover ⋯ menu on the conversation row. Rooms with many voices get a **roster panel** ("N IN THE ROOM →" in the header, or the "+N MORE" chip on the models strip): a scrollable list of everyone present — click a person for their full profile card, click their tier pill to cycle Haiku → Sonnet → Opus.
  - **@mention autocomplete.** Typing `@` in the composer pops a typeahead over the room's participants (keyboard or click to select); ambiguous first names insert the full name so mentions resolve unambiguously.
  - **"Build the room" browser.** `/conversations/new` is the full-page way to cast a chat: the library's smart search + filter rail + pagination with multi-select cards and a sticky launch bar (picks persist across searches and pages; ≤20). The quick picker stays for fast starts and links here ("BROWSE ALL WITH FILTERS →"); launching routes through `/conversations?draft=<keys>`.
  - **Rendering & history.** Agent replies render markdown (bold, headers, lists, inline code — `components/app/Markdown.tsx`, dependency-free, mention-aware). The sidebar shows only as many threads as fit its height (no overscroll) with SEE ALL → `/conversations/history`, a searchable list of every thread (title/person/role) that deep-links back via `/conversations?open=<id>`.
  - **@mentions direct the room.** Tagging `@Name` makes those personas answer (no cap — five mentions, five replies); with no tag, a Haiku-tier router picks the most relevant participant(s), or ALL of them when the message addresses the whole group ("each of you", "everyone"). Repliers see the full transcript, including each other's replies in the same round, and never speak for one another. Replies stream over ND-JSON as each author finishes, with "X & Y are typing…" indicators for whoever is still composing.
  ...
  - **Attachments (shipped).** Images and PDFs attach to messages (up to 8 per message, 5MB each, ~20MB combined — sized to Claude's 32MB request limit after base64 inflation), stored in the org's Storage folder, and passed to responders as native content blocks — experts genuinely analyze the file, not a filename. Chips UX: ≤3 files render as overlapping circles on the bubble; >3 collapse to two chips plus a "+N" expander that spreads all chips across the message top with a "−" to collapse. Click a chip for a lightbox (images) or signed-URL open (PDFs). Transcript history references attachments by name.
  - **Per-participant model tier (shipped).** A MODELS strip under the thread header shows one chip per participant; clicking opens the tier menu — Haiku 4.5 (lightweight & fast, the default), Sonnet 5 (balanced), Opus 4.8 (frontier reasoning) — from `lib/chat-models.ts` (the single source of chat model ids; never hardcode). Choices persist per conversation (`conversations.model_overrides` jsonb) and the converse route applies them per responder, logging the actual model used.
  - **Remix & full editor (shipped).** Any persona — library or custom — can be **remixed** into the org's custom library: every field editable (identity, tagline, demographics, skills, stances, trait sliders), saved with `spec.lineage` (ancestors oldest-first). Remixes carry a ⑂ REMIX tag and a clickable lineage chain in the profile ("Delores V. → Casey R. → Jordan P."); remixing a remix extends the chain. Custom personas are editable after creation; changing a name auto-rewrites first-name references in backstory/tagline/stances.
  - **Observability (shipped).** Every model call — router and replies — is logged to `agent_interactions` (surface, agent, model, tokens in/out, latency, status/error) under org RLS, surfaced in the in-app **Monitoring** page (stat tiles + interaction table + estimated spend). The engine logs simulation calls to the same table; keep the estimated-spend rates table in sync with Anthropic pricing.
  - **Agent Library UI (shipped).** The library tab lists the full generated library (§3.3) with live count; every card opens a full profile view — demographics grid (age, location, experience, credentials, household, income, tenure, occupation), backstory, standing positions, skills, trait bars, provenance line ("LIBRARY PERSONA · SYNTHETIC COMPOSITE"), start-conversation CTA. The same smart search powers the library page and the Conversations participant picker (local/custom personas filter instantly; the global library streams in debounced). The library page adds a **filter rail** (kind · category · age band · tenure · sort, with facet counts from `library_facets()`) that composes with the tab and the query, plus 24-per-page pagination — `search_personas` handles filters/sort/offset and returns a windowed total, and the client caches the LLM query parse so paging never re-runs the model.
  - **Roadmap (Conversations v2):** document attachments flowing through the corpus pipeline for RAG at scale (§2 Stage 2); chart generation as a tool call rendering customizable charts in-thread; per-persona long-term memory across threads (opt-in); group sessions upgraded onto InteractiveGroupChat when the engine lands.
...
| Interaction density | focused · lively · bustling (replies/crossfire/counters/interjections scale — §4.2) | lively |
...
**Interaction density** — how busy each round gets: `focused` (the tight v1 rhythm — ≤6 Agora replies, no crossfire/counters/interjections), `lively` (default — Agora replies ~1.5× the panel, Roundtable crossfire for half the panel, 2 Tribunal counters, 3 crowd interjectors per round), `bustling` (2× replies, full-panel crossfire, 4-shot counter-volley, 6 interjectors). Replies target ANY post from the round — weighted toward recent + contested — so chains form instead of a strict relay. Jury is exempt from extra deliberation posts (verdict integrity is the mode). The density math lives ONCE in `lib/run.ts` (agoraReplies/crossfireSlots/counterSlots/burstSize) — the engine and the cost estimator both read it, so they cannot drift.
...
> **Engine v1 deviation (shipped):** the POC engine is an **in-app TypeScript orchestrator** (`lib/engine.ts` + `/api/simulations/[id]/run/launch`, ND-JSON streaming) implementing all seven §5 choreographies, §6.2 event shapes, corpus grounding with citations (same Files-API path as Test-the-corpus, cache_control on the prefix), batched crowd sentiment polls, and the stability convergence check — persisting every post/event under RLS (migration 0012). `compilePersonaPrompt()` is the pure §6.1 compiler. Mode semantics: **round modes** (Agora, Roundtable, Tribunal, Jury) honor `rounds` + the stop rule and poll the crowd each round — Jury runs its rounds as MoA layers (blind scores → code-computed TALLY → re-score, convergence = score movement, computed in arithmetic); **fixed-shape modes** (Chamber, Desk, Expedition) execute their phases once and report `stop_reason: "choreography"` — the word "converged" is reserved for the stability rule actually firing (`stop_reason` lives in `run_result` + report methodology). Chamber polls after takes and after synthesis; Desk/Expedition don't poll. The Python/swarms service remains the scale path and takes over when runs outgrow serverless limits (~5 min); the event contract is identical by design so it swaps in behind the same run screen. **Living-forum layer (shipped):** replies thread to any post from the round (`pickReplyTarget`, recent+contested weighting), density scaling per §4.2, crowd interjection bursts (one batched call → INTERJECTION posts threaded under the round), an in-character **vote layer** (`post_votes` table + `votes` events; realtime micro-passes every 3rd post at lively/bustling + a full sweep at the round close, pair-deduped; totals feed the report as citable endorsement signals), **speaker guardrails** (never the same voice back-to-back, never a self-reply, coverage beats relevance — quiet leads get the mic; near-duplicate drafts get one do-not-restate retry via `textSimilarity`), and **Take the Floor** (`takeTheFloor()` + `/api/simulations/[id]/floor` — user posts persist as `author "user"`, tag FLOOR, mentioned agents reply in context). Every round-close artifact is resume-safe: polls/bursts/votes are deduped by round on continuation.
...
**Phase 3 (commercial data, per-org API keys):** ATTOM / CoreLogic-class property data & comps · Zillow research series · CoStar-class CRE comps · AcreValue-class farmland values · third-party AVM outputs (always treated as *inputs the panel argues with*, never ground truth) · permits (Shovels-class) · Placer-class foot traffic · school ratings.

**North star for agent intelligence:** the smartest agents possible for every real-estate question means aggressively expanding this tool/data surface — each new tool compounds across all 1,800+ personas and every decision shape. Tool access + the seeding corpus (§3.6) + calibration outcomes are the three data moats; prioritize tool phases accordingly.
