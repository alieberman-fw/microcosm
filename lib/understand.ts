...
export function understandSystem(docNames: string[]): string {
  ...
  return (
    ...
    `- poll_plan: 0-3 angles of this brief with a GENUINE preference/sentiment surface, ordered by phase: early = the broad gut-read (proposition), middle = the per-entity choice ("which category most deserves pursuit?" with the entities as options), late = the decision-shaped closer (proposition on the recommendation).\n` +
    `- BEFORE writing an empty poll_plan, look UNDER the stated questions: is there a POPULATION WHOSE BEHAVIOR this decision depends on — renters, buyers, tenants, neighbors, shoppers, guests, employees, occupiers? A brief can be expert-shaped on the surface and still turn on crowd behavior underneath ("conversion pencils only if the neighborhood turns residential" turns on whether renters would actually live there — poll THAT). When such a population exists, write the BEHAVIORAL question its members answer about their own lives (would you rent / pay / renew / visit / allow) — NEVER the user's decision itself ("convert vs hold" is underwriting; polling laypeople on it is noise). If you wrote population_hints cohorts, the poll plan is almost never empty — those cohorts exist because their behavior matters.\n` +
    `- An empty poll_plan is reserved for briefs with genuinely NO such population (pure pricing, document diligence, tax or capital-markets strategy, internal selections) — there it is a DECISION, not an omission.\n` +
